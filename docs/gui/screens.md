...
屏幕通常是Minecraft中所有图形用户界面（GUI）的基础：接收用户输入，在服务器上验证，并将生成的操作同步回客户端。它们可以与[菜单（Menus）][menus]相结合，为类似物品栏的视图创建通信网络，也可以是独立的，模组开发者可以通过自己的[网络][network]实现来处理。
...
每当用户做出关闭当前屏幕的输入时，就会调用`#onClose`。此方法通常用作回调，以销毁和保存屏幕本身中的任何内部进程。这包括向服务器发送数据包。
